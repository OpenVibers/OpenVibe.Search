...
- sitemaps, feeds or page caches (each product; they listen to `search.document.removed`)
...
JavaScript (there is no HTML search page yet — only the JSON API and a text index at `/`); real
persistence and end-to-end workflows; capabilities and events registered in OpenVibe.Contracts; a
security review; and acceptance tests. The launch release removes the domain from
`OpenVibe.Sites/sites.json` atomically.
